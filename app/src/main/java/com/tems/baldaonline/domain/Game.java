                            ...
                            if(!firstUser.getWords().contains(word) && !secondUser.getWords().contains(word)&& !startWord.equalsIgnoreCase(word)){// проверка на повторение
                                new DialogYesNo(null , () -> {
                                    wordsRepository.getWord(word, data -> {
                                        if (data.getWord() != null) {  //есть ли слово в словаре// сохраняем введенное слово///////////////////////////////////////////////////////////////////////////////////
                                            ...
                                            if (countTurns == rowsCount * (rowsCount - 1)) {
                                                if (firstUser.getCount() != secondUser.getCount()) {
                                                    ...
                                                    new DialogFinish(() -> new DialogYesNo(null, () -> context.startActivity(new Intent(context, ActivityMenu.class)), "Выйти в меню?").show(manager, "chase_dialog"), winUser).show(manager, "finish_dialog");
                                                } else {
                                                    Log.d(myTag, "ничья");
                                                }
                                            }
